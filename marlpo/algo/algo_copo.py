...
from algo.algo_ccppo import CCModel, CCPPOConfig, CCPPOPolicy, CCPPOTrainer
    ...
    def postprocess_trajectory(self, sample_batch, other_agent_batches=None, episode=None):
        ...
        with torch.no_grad():
            ...
            if episode is not None:  # After initialization
                ...
                fix_a_bug_in_sample_batch(sample_batch)
                ...
                try:
                    ...
                    print('******------', 'info keys', type(infos[0]), infos[0].keys())
                    print('******------', infos[0][0])
                    print('******------', episode, len(sample_batch))
                    # print('******------', [info[NEI_REWARDS] for info in infos])
        ...
        assert sample_batch["step_lcf"].max() == sample_batch["step_lcf"].min(), (sample_batch["step_lcf"].max(), sample_batch["step_lcf"].min())
        return sample_batch
...
class CoPOTrainer(CCPPOTrainer):
    ...
    def training_step(self):
        ...
        local_policy = self.get_policy(DEFAULT_POLICY_ID)
        # wrapped = False
        ...
        for i in range(self.config["lcf_num_iters"]):
            ...
            for mb_id, minibatch in enumerate(minibatches(train_batch.policy_batches[DEFAULT_POLICY_ID],
        ...
        train_results[DEFAULT_POLICY_ID]["custom_metrics"]["meta_update"] = meta_update_fetches
...
def fix_a_bug_in_sample_batch(sample_batch: SampleBatch):
    infos = sample_batch[SampleBatch.INFOS] # ndarray of dict
    nei_rewards = []
    ...
    for i, info in enumerate(infos):
        ...
        # special case for a RLlib bug that when env reset, the 1st info (infos[0]) may be {0: {'agent0': ..., 'agent1': ...}}
        if NEI_REWARDS not in info:
            # logger.warning(
            #     "No 'nei_rewards' key in info!!!"
            # )
            assert set(info.keys()) == set([0])
            assert sample_batch['t'][0] == 0
            keys = [info.keys() for info in infos] # [0, {single agent info keys}, ...]
            assert [0] == [i for i, k in enumerate(keys) if k == set([0])] # 只在sample_batch的infos里的第一个位置出现
                
            agent_id = f"agent{sample_batch['agent_index'][0]}"
            true_info = info[0][agent_id]
            # logger.warning(f'Find true info for {agent_id}')
    ...
    if true_info is not None:
        infos[0] = true_info
        sample_batch[SampleBatch.INFOS] = infos
