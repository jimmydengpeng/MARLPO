...
from env.env_wrappers import NeighbourEnv
...
class CCEnv:
    ...
    def _get_reset_return(self):
        ...
        for kkk in i.keys():
            ...
            i[kkk]['episode_distance'] = 0 # just for COMPATIBILITY to TrackingEnv(tracking distance)!!!
        return obs, i
    ...
    def step(self, actions):
        ...
        o, r, tm, tc, i = super(CCEnv, self).step(actions)
        self._update_distance_map(dones=tm)
        for kkk in i.keys():
            i[kkk]["agent_id"] = kkk
            i[kkk]["all_agents"] = list(i.keys())
...
class LCFEnv(CCEnv):
# class LCFEnv(NeighbourEnv):
    @classmethod
    def default_config(cls):
        config = super(LCFEnv, cls).default_config()
        # Only For CCEnv Compatible! Comm features will not be used!
        # config.update(
        #     dict(
        #         communication=dict(comm_method="none", comm_size=4, comm_neighbours=4, add_pos_in_comm=False),
        #         add_traffic_light=False,
    ...
    def _get_reset_return(self):
        ...
        obses, i = super(LCFEnv, self)._get_reset_return()
        ...
        for k, o in obses.items():
            ...
            self.lcf_map[k] = lcf
            # ======== add to info =====
            if k not in self.lcf_map:
                # The agent LCF is set for the whole episode
                self.lcf_map[k] = lcf
            i[k]["lcf"] = lcf
            i[k]["lcf_deg"] = lcf * 90

            i[k]["coordinated_rewards"] = 0.0
            i[k]["native_rewards"] = 0.0
        ...
        for agent_name, agent_info in i.items():
            ...
            i[agent_name]["nei_rewards"] = 0.0
            i[agent_name]["global_rewards"] = 0.0

        return ret, i

    def step(self, actions):
        # step the environment
        o, r, tm, tc, i = super(LCFEnv, self).step(actions)
...
# From CoPO, not in use!
# Re-Implemented in env_wrappers!
...
if __name__ == '__main__':
    ...
    for _ in trange(10):
        for _ in trange(1000):
            o, r, tm, tc, i = env.step({k: [0, 1] for k in env.vehicles})
            # print(d)
