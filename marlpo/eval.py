...
def get_env(env, should_wrap_copo_env=False, should_wrap_cc_env=False, svo_mean=0.0, svo_std=0.0):
...
def get_env_and_start_seed(trial_path):
    ...
    env_name = param["env"] # may be abbreviation, e.g. 'Intersection'
...
if __name__ == '__main__':
    ...
    for pi, (trial_path, trial_name) in enumerate(paths):
        ...
        for ckpt_path, ckpt_idx in ckpt_paths:
            ...
            print(
                f"We will evaluate checkpoint: Algo-{algo_name}, Env-{raw_env_name}, Seed-{start_seed}, "
                f"Ckpt-{ckpt_idx}"
            )
        ...
        try:
            ...
            while True:
                ...
                if tm["__all__"]:
                    ...
                    print(
                        printPanel(res, title=f'=== Evaluation Result for Episode {ep_count}/{num_episodes} {result_name}')
                    )
