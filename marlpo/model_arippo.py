...
from marlpo.utils.debug import log, inspect
...
class ARFullyConnectedModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        # print('>>> [ARModel]')
        self.n_actions = model_config['custom_model_config']['n_actions']
        ...
        prev_layer_size = int(np.product(obs_space.shape)) \
                        + int(np.product(action_space.shape)) * (self.n_actions)
        self._logits = None
    ...
    def forward(
        ...
        # concat with obs
        assert self._last_flat_in.shape[0] == other_actions.shape[0]
        hidden_layer_in = torch.cat((self._last_flat_in, other_actions), 1)
        # inspect(hidden_layer_in.shape)
        self._features = self._hidden_layers(hidden_layer_in)
